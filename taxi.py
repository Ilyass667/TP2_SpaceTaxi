...
class Taxi(pygame.sprite.Sprite):
    ...
    _MAX_ACCELERATION_Y_DOWN = 0.005 

    _MAX_VELOCITY_SMOOTH_LANDING = 0.5 # vitesse maximale permise pour un atterrissage en douceur  0.50
    _MAX_VELOCITY_ROUGH_LANDING = 4.0
    _FRICTION_MUL = 0.9995  # la vitesse horizontale est multipliée par la friction
    _GRAVITY_ADD = 0.001 # la gravité est ajoutée à la vitesse verticale  0.005
...
#------------------------------------------------------------------------------------------------
    def land_on_pad(self, pad: Pad) -> bool:
        ...
        if not self.rect.colliderect(pad.rect):
            return False
        # Calculer les positions des deux pattes du taxi
    ...
    def _reinitialize(self) -> None:
        ...
        # M1 remplacer self._pos_x et self._pos_y par un objet pygame.Vector2.
